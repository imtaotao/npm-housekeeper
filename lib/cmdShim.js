...
const chmodShim = (to) => {
  ...
  fs.chmodSync(to + ".cmd", 0o755);
  fs.chmodSync(to + ".ps1", 0o755);
};
...
const writeShim = (from, to) => {
  ...
  try {
    ...
    writeShim_(from, to, prog, args, vars);
  } catch (e) {
    writeShim_(from, to);
  }
};
...
const writeShim_ = (from, to, prog, args, variables) => {
  ...
  fs.writeFileSync(to + ".ps1", pwsh, "utf8");
  fs.writeFileSync(to + ".cmd", cmd, "utf8");
  fs.writeFileSync(to, sh, "utf8");
  chmodShim(to);
};
