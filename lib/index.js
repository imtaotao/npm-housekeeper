...
const { httpClient, simpleFetch } = require("./utils");
...
exports.Installer = class Installer {
  ...
  setupLinks(manager) {
    // workspace
    for (const [_, node] of Object.entries(manager.workspace)) {
    ...
    manager.each((name, version, node) => {
      for (const [_, edge] of Object.entries(node.edges)) {
        ...
        const destDir = this.modulesDir(`${name}@${version}`);
  ...
  download(name, version, resolved, integrity, destDir) {
    return new Promise(async (resolve, reject) => {
      const cacheDir = path.resolve(
        this.cacheDir,
        `${name}@${version}_${integrity}.tgz`
      );
  ...
  GeneratePackages(allPackages, { remove }) {
    const each = (packages, cb) => {
      const ls = [];
      for (const name in packages) {
        for (const version in packages[name]) {
          const dir = this.modulesDir(`${name}@${version}`);
          const { resolved, integrity } = packages[name][version];
          if (fs.existsSync(dir)) {
            fs.rmSync(dir, { recursive: true, force: true });
          }
          cb && ls.push(cb(name, version, resolved, integrity, dir));
        }
      }
      return ls;
    };
    ...
    each(remove);
    // add
    const ls = each(allPackages, this.download.bind(this));
  ...
  async start() {
    ...
    await this.GeneratePackages(manager.packages, diffData);
