...
exports.Installer = class Installer {
  ...
  setupBinScripts(node, dir) {
    // TODO: .bin
  }

  setupLinks(manager) {
    const set = (targetDir, destDir, node) => {
      ...
      this.setupBinScripts(node, targetDir);
    };
    ...
    for (const [_, node] of Object.entries(manager.workspace)) {
      ...
      for (const [_, edge] of Object.entries(node.edges)) {
        ...
        set(targetDir, destDir, edge.node);
      }
    }
      ...
      for (const name in packages) {
        for (const version in packages[name]) {
          ...
          if (type === "remove") {
            ...
            ls.push(download(name, version, resolved, integrity, dir));
          }
        }
      }
      return ls;
